...
% (c) Gløersen Ø, Federolf P (2016) Predicting Missing Marker Trajectories in Human Motion Data Using Marker Intercorrelations. PLoS ONE 11(3): e0152616. https://doi.org/10.1371/journal.pone.0152616
...
%% modification: if all markers are compromised, the weighted nanmean will be computed for the mean_trajectory (otherwise it won't work)
if length(ColumnsWithGaps) == size(Data_gaps,2)
else DataWithoutCompromizedMarkers(:,ColumnsWithGaps)=[];
end

%% Subtract average marker trajectory (to obtain a coordinate system moving with the subject)
%% modification: compute mean_trajectory with weighted nanmean (weight = percentage of a single channel of all nans)
if length(ColumnsWithGaps) == size(Data_gaps,2)
    % determine weights according to the amount of nans in one column compared to all nans in all columns
    markerWeights = sum(isnan(Data_gaps),1)/sum(isnan(Data_gaps),[1,2]);

    % calculate the averages omitting nan-values with a weight of 1-(sum of nans in a colum / sum of all nans)
    mean_trajectory.x = ...
        nanmean(DataWithoutCompromizedMarkers(:,...
        1:3:columns).*(1-markerWeights(1:3:columns)*3),2);
    mean_trajectory.y = ...
        nanmean(DataWithoutCompromizedMarkers(:,...
        2:3:columns).*(1-markerWeights(2:3:columns)*3),2);
    mean_trajectory.z = ...
        nanmean(DataWithoutCompromizedMarkers(:,...
        3:3:columns).*(1-markerWeights(3:3:columns)*3),2);
else mean_trajectory.x = ...
...
function [distArray] = distance2marker(MarkerData,colWithGaps)
    ...
    for i=1:n
        ...
        catch disp(['problem for timepoint ' num2str(i)])
        end
    end
